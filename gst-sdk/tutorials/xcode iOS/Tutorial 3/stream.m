...
void *app_function(void *userdata)
{
	...
	if (mLevel == 1) {
		videoThread = g_thread_new("video thread level 1", level_1, NULL);
	} else if (mLevel == 2) {
		videoThread = g_thread_new("video thread level 2", level_2, NULL);
	}
...
void out_of_current_session()
{
	...
	sprintf(sendBuffer, "<DESTROYCONN>" "<from>%s</from>" "</DESTROYCONN>", username);
