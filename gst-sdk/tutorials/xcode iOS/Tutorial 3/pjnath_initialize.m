...
static pj_status_t holder_init(Holder *holder)
{
    ...
    holder->pool = pj_pool_create(&holder->cp.factory, "holder",
                                  512, 512, NULL);
...
static void holder_input_remote(const char *ice_sdp, Holder *holder)
{
    ...
    if (comp0_port==0 || comp0_addr[0]=='\0') {
        ...
        status = pj_sockaddr_set_str_addr(af, &holder->rem.def_addr[0],
                                          &tmp_addr);
        if (status != PJ_SUCCESS) {
