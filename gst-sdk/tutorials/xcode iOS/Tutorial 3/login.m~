...
int connect_with_timeout(char *host, int port, int timeout_sec, int timeout_usec, char *account)
{
	int res;
	int valopt;
...
int login_to_server(char *username, char *password)
{
	...
	sprintf(infoBuf, "<LOGIN>"
		"<devType>APP</devType>" "<ID>%s</ID>" "<password>%s</password>" "</LOGIN>", username, password);

	retValue = connect_with_timeout(SERVER, SERVER_PORT, timeout_second, timeout_microsecond, infoBuf);
	free(infoBuf);
	return retValue;
}
...
char *list_available_session()
{
	...
	sprintf(sender, "<LISTONLINESESSIONS>" "</LISTONLINESESSIONS>");
...
int join_into_session(char *sessionId)
{
	...
	sprintf(sendBuf, "<JOINSESSION>" "<sessionID>%s</sessionID>" "</JOINSESSION>", sessionId);
